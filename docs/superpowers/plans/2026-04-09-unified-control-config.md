...
| `src/config/unifiedControl.ts` (new) | `tryParseUnifiedControlConfig(input: unknown): ControlConfig \| undefined` — root validation, module iteration, first-wins, invalid-property policy, gamification field sanitization (reuse helpers extracted from current `schema.ts`) |
