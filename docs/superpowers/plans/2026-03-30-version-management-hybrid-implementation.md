...
1. Extend integration config to include optional remote fields:
  - `gamification`: `packageVersion?: string; apiKey?: string`
  - `onesignal`: `{ enabled: boolean }` only in this pass (remote knobs are **gamification-only**).
...
- **Step 4: Implement wiring in `src/global.ts`**pluginOverrides

Update the `applyConfig()` inner function to call:
`await hub.setControlConfig(controlService.getConfig()!)`

The hub no longer wires automatic SPA `history` listeners; consumers use `CExP.page()` where needed.
...
- For vendor changes that require script URL/path changes (e.g. fixed OneSignal or gamification defaults in this repo), a new `cexp-hub-sdk` release is still required per the hybrid design.
